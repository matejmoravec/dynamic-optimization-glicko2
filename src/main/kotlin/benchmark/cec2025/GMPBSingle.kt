...
import util.GMPBConfig

class GMPBSingle(val config: GMPBConfig, val maximization: Boolean = true, val drawLimit: Double = 1e-7) :
    ...
    override fun initAllProblems() {
        ...
        for (k in 0 until config.FEs.size) {
            if (config.FEs[k] % config.changeFrequency == 0) {
                ...
                //counter++
                //println("k=$k, FEs[k]=${config.FEs[k]}")
            }
        }
